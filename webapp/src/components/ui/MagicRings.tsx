...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const dr = Math.min(window.devicePixelRatio, 2);
      renderer.setSize(w, h);
      renderer.setPixelRatio(dr);
      uniforms.uResolution.value.set(w * dr, h * dr);
    };
    resize();
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      mouseRef.current[0] = (e.clientX - rect.left) / w - 0.5;
      mouseRef.current[1] = -((e.clientY - rect.top) / h - 0.5);
    };
  ...
  return <div ref={mountRef} className={`magic-rings-container ${className}`} style={combinedStyle} />;
}
